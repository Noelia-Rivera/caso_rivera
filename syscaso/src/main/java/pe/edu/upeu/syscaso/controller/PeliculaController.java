...
public class PeliculaController {
	...
	@GetMapping("/{id}")
	public ResponseEntity<Pelicula> getPeliculaId(@PathVariable Long id) {
		try {
			Pelicula p = service.read(id).get();
			return new ResponseEntity<>(p, HttpStatus.CREATED);
		} catch (Exception e) {
			// TODO: handle exception
			return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
	...
	@DeleteMapping("/{id}")
	public ResponseEntity<Pelicula> delPelicula(@PathVariable Long id) {
		try {
			service.delete(id);
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> updatePelicula(@PathVariable Long id, @Valid @RequestBody Pelicula pe) {
		Optional<Pelicula> p =service.read(id);
		if (p.isEmpty()) {
